...
def calculate_ema(file_path, alpha):
    # Load the CSV file
    ...
    data['EMA'] = [None] + ema_values[:-1]  # None for the first entry, shift EMA values
...
def plot_data_ema(data, alpha):
    ...
    # Plot actual values
    plt.plot(data['T'], data['Yt'], label='Actual Yt', color='blue', marker='o', linestyle='-')
    
    # Plot EMA
    plt.plot(data['T'], data['EMA'], label=f'EMA (alpha={alpha})', color='orange', marker='o', linestyle='-')
...
def main():
    ...
    # Calculate EMA
    try:
        result = calculate_ema(file_path, alpha)
        print(result)

        # Plot data
        plot_data_ema(result, alpha)
