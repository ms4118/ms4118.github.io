...
def predict_next_value(data):
    ...
    for i in range(len(data) - 3):
        ...
        next_value = (data['Yt'].iloc[i] + data['Yt'].iloc[i + 1] + data['Yt'].iloc[i + 2]) / 3
...
def plot_data(data, r, predicted_df):
    ...
    # Plot predictions
    plt.plot(predicted_df['T'], predicted_df['Predicted_Yt'], label='Predicted Yt', color='green', linestyle='--')
...
def main():
    ...
    # Calculate SMA
    try:
        result = calculate_sma(file_path, r)
        print(result)

        # Predict next values
        predicted_df = predict_next_value(result)

        # Plot data
        plot_data(result, r, predicted_df)
