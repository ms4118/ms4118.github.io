...
def predict_future_values_wma(data, r, forecast_periods):
    ...
    for _ in range(forecast_periods):
        ...
        last_values = list(last_values[1:]) + [next_value]  # Shift and append new prediction
...
def plot_data_wma(data, r, future_values):
    ...
    # Plot WMA
    plt.plot(data['T'], data['WMA'], label=f'WMA (window={r})', color='orange', marker='o')

    # Plot future predictions
    future_t = range(data['T'].iloc[-1] + 1, data['T'].iloc[-1] + len(future_values) + 1)
    plt.plot(future_t, future_values, label='Future Predictions', color='green', linestyle='--', marker='o')
    ...
    for i, value in enumerate(data['Yt']):
        plt.text(data['T'].iloc[i], value, f'{value:.2f}', fontsize=9, ha='right', va='bottom')

    # Label the WMA values
    for i, value in enumerate(data['WMA'].dropna()):
        plt.text(data['T'].iloc[i + r - 1], value, f'{value:.2f}', fontsize=9, ha='right', va='bottom')

    # Label the future predictions
    for i, value in enumerate(future_values):
        plt.text(future_t[i], value, f'{value:.2f}', fontsize=9, ha='right', va='bottom')
...
def main():
    ...
    forecast_periods = int(input("Enter the number of future periods to forecast: "))

    # Calculate WMA
    try:
        result = calculate_wma(file_path, r)
        print(result)
        ...
        # Plot data
        plot_data_wma(result, r, future_values)
        plt.show()
